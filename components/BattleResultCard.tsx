...
const BattleResultCard: React.FC<{ summary: TurnSummary }> = ({ summary }) => {
  ...
  return (
    <section className={`mt-3 w-full max-w-xl rounded border p-3 ${accent}`} aria-label={`${summary.title}结果`}>
      <div className="flex items-center justify-between gap-3 border-b border-neutral-800 pb-2">
        <h3 className="text-xs font-bold tracking-widest text-neutral-200">{summary.title}</h3>
        <span className="text-[10px] font-mono text-neutral-500">耗时 {formatDuration(summary.durationMinutes)}</span>
      </div>
      ...
      {summary.deltas.length > 0 && (
        <div className="mt-2 grid grid-cols-2 gap-1 sm:grid-cols-3">
          {summary.deltas.map((delta) => {
            ...
            return (
              <div key={delta.metric} className="flex items-center justify-between rounded bg-black/50 px-2 py-1 text-[10px]">
                <span className="text-neutral-500">{delta.label}</span>
      ...
      {summary.notes.length > 0 && (
        <div className="mt-2 space-y-0.5 text-[9px] leading-relaxed text-neutral-600">
